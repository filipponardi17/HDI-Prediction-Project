...
t2 = t1[1:25] #20 predictor in decreasing order
...
variables_to_keep <- c("js_hom", "age", "dut_ill", "js_abr", "js_div", "imp_rlg", "js_euth", "ps_arm", "el_fair", "el_brb", "ps_lead",
                   "job_men", "trs_ntn", "dmc_coup", "c_chr", "el_thr", "dmc_obey", "dut_chl", "hom_prn", "dmc_cnt", "dmc_rlg", "trs_knw","c_gvr", "c_plc", "c_prt", "HDI")


# Create a new data frame with only the selected variables
subset_data <- subset_data[, variables_to_keep]
...
subset_data_1 <- subset_data[, c("js_hom", "js_abr", "js_euth", "hom_prn", "imp_rlg", "c_chr")]
subset_data_2 <- subset_data[, c("c_chr", "c_gvr", "c_plc", "c_prt")]
subset_data_3 <- subset_data[, c("el_fair", "el_brb", "el_thr", "dmc_cnt", "dmc_rlg", "c_gvr" )]
ggcorr(subset_data_3, label=TRUE)

#also other vars 
subset_data$rel= (subset_data$js_hom + subset_data$js_abr + subset_data$js_euth) /3
subset_data$conf= (subset_data$c_prt + subset_data$c_gvr) /2
...
```{r}
in_train <- createDataPartition(subset_data$HDI, p = 0.75, list = FALSE)  # 75% for training
training <- scaled_df[ in_train,]
testing <- scaled_df[-in_train,]
nrow(training)
nrow(testing)
...
linFit <- lm(HDI ~ js_hom, data=training)
summary(linFit)
```
...
model = HDI ~ js_hom + age + dut_ill + js_abr + js_div + imp_rlg + js_euth + ps_arm + el_fair + el_brb + ps_lead + job_men + trs_ntn + dmc_coup + c_chr + el_thr + dmc_obey + dut_chl + hom_prn + dmc_cnt + dmc_rlg + trs_knw + c_gvr + c_plc + c_prt
